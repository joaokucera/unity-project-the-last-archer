...
{
	public class JKFacebookUtil : ScriptableObject 
	{
		...
		public static Dictionary<string, string> DeserializeJSONProfile(string response)
		{
			...
			object firstNameH;
			if (responseObject.TryGetValue("first_name", out firstNameH))
			{
				profile["first_name"] = (string)firstNameH;
			}
			...
			object genderH;
			if (responseObject.TryGetValue("gender", out genderH))
			{
				profile["gender"] = (string)genderH;
			}
